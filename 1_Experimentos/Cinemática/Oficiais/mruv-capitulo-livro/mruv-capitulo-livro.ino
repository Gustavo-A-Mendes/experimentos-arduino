...
int iniciar = 0;
// variável que guarda os estados dos sensores
bool estadoInicial;
bool estadoInicialAnt = true;
bool estadoIR;
bool estadoIRAnt = true;
// variável de tempo, velocidade e aceleracao
unsigned long instanteInicial, instanteIR;          // tempo decorrido pelo programa até o sensor
unsigned long interTempoInicial, interTempoIR;      // intervalo decorrido do início até o sensor
  ...
  while (inicioAtivo == HIGH) {
    ...
    if (estadoIR != estadoIRAnt) {
      if (estadoIR == HIGH) {
        ...
        // calcular velocidade pela área do trapézio
        velocidade = (0.002 * deslocamentoIR) / (0.001 * interTempoIR);
        aceleracao = sq(velocidade) / (0.002 * deslocamentoIR);
        
        // aceleracao = (0.002 * deslocamentoIR) / sq(0.001 * interTempoIR);
