...
int rodada = 0;
int rodadasParaVencer = 15;
long tempoLimite = 3000;


int azul, amarelo, verde, vermelho;
...
int jogo(void) {
  ...
  while (rodada <= rodadasParaVencer) {
    mostrarSequencia();
...
void gerarSequencia(void) {
  for (int i = 0; i < rodadasParaVencer; i++) {	
...
int checaBotao(void) {
  long x = analogRead(A5);
  if (x >= 80 && x < 300) return (CHOICE_VERMELHO);
  else if (x >= 300 && x < 550) return (CHOICE_VERDE);
  else if (x >= 550 && x < 800) return (CHOICE_AMARELO);
  else if (x >= 800) return (CHOICE_AZUL);
